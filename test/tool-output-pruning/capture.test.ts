...
function makeAssistantMessage(toolCalls?: Array<{ id: string; name: string }>): AgentMessage {
	return {
...
function makeToolResult(options: {
	...
	if (options.image) {
		content.push({ type: "image", source: { type: "base64", media_type: "image/png", data: "abc" } });
	}
	if (options.mixed) {
		content.push({ type: "text", text: options.text ?? "" });
		content.push({ type: "image", source: { type: "base64", media_type: "image/png", data: "abc" } });
...
describe("extractToolResultText", () => {
	it("extracts text from a text-only tool result", () => {
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "hello world" });
	...
	it("returns empty string for non-toolResult", () => {
		const msg = { role: "user" as const, content: "hello", timestamp: Date.now() } as unknown as AgentMessage;
		expect(extractToolResultText(msg)).toBe("");
	});

	it("returns empty string for missing content", () => {
		const msg = { role: "toolResult" as const, toolCallId: "tc1", toolName: "bash", timestamp: Date.now() } as unknown as AgentMessage;
		expect(extractToolResultText(msg)).toBe("");
	});

	it("ignores image blocks", () => {
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", image: true });
		expect(extractToolResultText(msg)).toBe("");
	});
});
...
describe("isTextOnlyToolResult", () => {
	it("returns true for text-only", () => {
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "hello" });
	...
	it("returns false for mixed content", () => {
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "hello", mixed: true });
		expect(isTextOnlyToolResult(msg)).toBe(false);
	});
	...
	it("returns false for truly empty content array", () => {
		const msg = { role: "toolResult" as const, toolCallId: "tc1", toolName: "bash", content: [] as unknown[], isError: false, timestamp: Date.now() } as unknown as AgentMessage;
		expect(isTextOnlyToolResult(msg)).toBe(false);
	});

	it("returns false for non-toolResult", () => {
		expect(isTextOnlyToolResult({ role: "user" as const, content: "hello", timestamp: Date.now() } as unknown as AgentMessage)).toBe(false);
	});
});
...
describe("isEligibleToolResult", () => {
	...
	it("rejects mixed content", () => {
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "x".repeat(3000), mixed: true });
		expect(isEligibleToolResult(msg, DEFAULT_SETTINGS)).toBe(false);
	});

	it("rejects results below min chars", () => {
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "short" });
		expect(isEligibleToolResult(msg, DEFAULT_SETTINGS)).toBe(false);
	});

	it("rejects non-toolResult messages", () => {
		expect(isEligibleToolResult({ role: "user" as const, content: "hello", timestamp: Date.now() } as unknown as AgentMessage, DEFAULT_SETTINGS)).toBe(false);
	});
	...
	it("honors included-tools filter when non-empty", () => {
		...
		const text = "x".repeat(3000);
		expect(isEligibleToolResult(makeToolResult({ toolCallId: "tc1", toolName: "bash", text }), settings)).toBe(false);
		expect(isEligibleToolResult(makeToolResult({ toolCallId: "tc2", toolName: "web_search", text }), settings)).toBe(true);
	});
});

describe("buildArgsPreview", () => {
	it("returns null when details are absent", () => {
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "out" });
		expect(buildArgsPreview(msg)).toBeNull();
	});

	it("returns JSON string for details", () => {
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "out", details: { command: "ls" } });
		expect(buildArgsPreview(msg)).toBe('{"command":"ls"}');
	});

	it("truncates long previews", () => {
		const long = "x".repeat(500);
		const msg = makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "out", details: { data: long } });
...
describe("captureBatch", () => {
	...
	it("returns null for non-assistant message", () => {
		const msg = { role: "user" as const, content: "hello", timestamp: Date.now() } as unknown as AgentMessage;
		const result = captureBatch(msg, [], 0, Date.now(), DEFAULT_SETTINGS, state);
	...
	it("returns null when no tool results are eligible", () => {
		...
		const results = [
			makeToolResult({ toolCallId: "tc1", toolName: "read", text: "x".repeat(3000) }),
		];
		const result = captureBatch(assistant, results, 0, Date.now(), DEFAULT_SETTINGS, state);
		expect(result).toBeNull();
	});

	it("captures only eligible tool results", () => {
		const assistant = makeAssistantMessage([{ id: "tc1", name: "bash" }, { id: "tc2", name: "read" }]);
		const results = [
			makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "x".repeat(3000) }),
			makeToolResult({ toolCallId: "tc2", toolName: "read", text: "x".repeat(3000) }),
		];
		const result = captureBatch(assistant, results, 1, 1000, DEFAULT_SETTINGS, state);
	...
	it("assigns sequential short refs", () => {
		const assistant = makeAssistantMessage([{ id: "tc1", name: "bash" }, { id: "tc2", name: "bash" }]);
		const results = [
			makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "x".repeat(3000) }),
			makeToolResult({ toolCallId: "tc2", toolName: "bash", text: "x".repeat(3000) }),
		];
		const result = captureBatch(assistant, results, 0, 1000, DEFAULT_SETTINGS, state);
	...
	it("sets correct metadata on records", () => {
		...
		const text = "x".repeat(3000);
		const results = [
			makeToolResult({ toolCallId: "tc1", toolName: "bash", text, isError: true, details: { cmd: "ls" } }),
		];
		const result = captureBatch(assistant, results, 2, 5000, DEFAULT_SETTINGS, state);
	...
	it("populates argsPreview and fallbackSnippets when available", () => {
		...
		const text = "x".repeat(3000);
		const results = [
			makeToolResult({ toolCallId: "tc1", toolName: "bash", text, details: { command: "echo hi" } }),
		];
		const result = captureBatch(assistant, results, 0, 1000, DEFAULT_SETTINGS, state);
		const record = result!.records[0];
...
describe("serializeBatchForSummarizer", () => {
	it("serializes records with bounded text", () => {
		...
		const toolResults = [
			makeToolResult({ toolCallId: "tc1", toolName: "bash", text: "hello world" }),
		];
		const text = serializeBatchForSummarizer(records, toolResults, DEFAULT_SETTINGS);
