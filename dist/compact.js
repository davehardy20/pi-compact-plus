...
const SECTION_BODY_LINE_LIMITS = new Map([
    ["## Current Objective", 4],
...
function estimateSummaryTokens(text) {
    return text.length / 4;
}
function truncateLine(line, maxChars = MAX_SUMMARY_LINE_CHARS) {
...
function normalizeStructuredSummary(summary, maxTokens, targetTokens) {
    ...
    const rebuild = (multiplier) => {
        const rendered = [];
        for (const section of sections) {
            rendered.push(section.heading);
            const bodyLimit = Math.max(2, Math.floor((SECTION_BODY_LINE_LIMITS.get(section.heading) ?? 6) * multiplier));
            const body = [];
            ...
            for (const rawLine of section.body) {
                ...
                if (isBlank) {
                    if (previousBlank || body.length === 0)
                        continue;
                    previousBlank = true;
                    body.push("");
                    continue;
...
function normalizePreviousSummary(previousSummary) {
    if (!previousSummary)
        return previousSummary;
    return normalizeStructuredSummary(previousSummary, MAX_PREVIOUS_SUMMARY_TOKENS, TARGET_PREVIOUS_SUMMARY_TOKENS);
}
function normalizeCompactionResult(result) {
    const summary = result.summary ?? "";
    const normalizedSummary = normalizeStructuredSummary(summary, MAX_VALID_SUMMARY_TOKENS, TARGET_NORMALIZED_SUMMARY_TOKENS);
 ...
 */
function validateCompactionResult(result) {
    const summary = result.summary ?? "";
    if (summary.length === 0) {
    ...
    if (summary.length > 100) {
        ...
        const foundHeadings = expectedHeadings.filter((h) => summary.includes(h)).length;
        if (foundHeadings < 2) {
            return {
                valid: false,
 ...
 */
function restoreToolPairs(pruned, original) {
...
export async function runCustomCompaction(preparation, mode, ctx, signal) {
    try {
        ...
        if (mode === "hard") {
            ...
            const classifiedHistory = classifyMessages(preparation.messagesToSummarize, mode);
            const prunedHistory = [
                ...classifiedHistory.critical,
                ...classifiedHistory.contextual,
            ];
            const historyOrderMap = new Map(preparation.messagesToSummarize.map((m, i) => [m, i]));
            prunedHistory.sort((a, b) => {
                const idxA = historyOrderMap.get(a);
                const idxB = historyOrderMap.get(b);
                return (idxA ?? 0) - (idxB ?? 0);
            });
            const historyWithPairs = restoreToolPairs(prunedHistory, preparation.messagesToSummarize);
            // Prune ephemeral from turn prefix when splitting
            let prunedPrefix;
            if (preparation.isSplitTurn &&
                preparation.turnPrefixMessages.length > 0) {
                const classifiedPrefix = classifyMessages(preparation.turnPrefixMessages, mode);
                const pruned = [
                    ...classifiedPrefix.critical,
                    ...classifiedPrefix.contextual,
                ];
                const prefixOrderMap = new Map(preparation.turnPrefixMessages.map((m, i) => [m, i]));
                pruned.sort((a, b) => {
            ...
            const prefixWithPairs = prunedPrefix && preparation.turnPrefixMessages.length > 0
        ...
        const normalizedPreviousSummary = normalizePreviousSummary(preparation.previousSummary);
        preparation = {
            ...preparation,
        ...
        const result = await compact(preparation, model, auth.apiKey ?? "", auth.headers, customInstructions, signal ?? ctx.signal ?? undefined);
        // Compute classified counts for telemetry
        const classified = classifyMessages(mode === "hard" ? preparation.messagesToSummarize : focusSource, mode);
