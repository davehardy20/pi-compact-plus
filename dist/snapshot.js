import { extractActiveFiles, extractBlockers, extractDecisions, extractDependencyChain, extractObjective, extractTextContent, isConversationalFiller, } from "./extract.js";
/**
...
export function extractCurrentFocus(messages) {
 ...
 */
export function extractSessionSnapshot(messages) {
    const recent = messages.slice(-20);
...
export function extractCompletedWork(messages) {
    const items = [];
    for (const msg of messages) {
        const text = extractTextContent(msg);
        // Inline markdown checklist items: [x] ...
        const checklistPattern = /\[x\]\s*(.+)/g;
        let match = checklistPattern.exec(text);
        while (match !== null) {
        ...
        const sectionMatch = text.match(/##\s*(?:completed work|done|finished|implemented)\s*\n((?:[-*]\s+.+\n?)+)/i);
        if (sectionMatch) {
            for (const line of sectionMatch[1].split(/\n/)) {
                const item = line.replace(/^[-*]\s+/, "").trim();
                if (item.length > 5)
                    items.push(item.slice(0, SNAPSHOT_MAX_LINE));
            }
        }
    }
    return Array.from(new Set(items)).slice(-SNAPSHOT_MAX_ITEMS);
}
export function extractOpenProblems(messages) {
    const items = [];
    for (const msg of messages) {
        const text = extractTextContent(msg);
        // Inline patterns
        ...
        const sectionMatch = text.match(/##\s*(?:open problems?|todo|remaining|outstanding)\s*\n((?:[-*]\s+.+\n?)+)/i);
        if (sectionMatch) {
            for (const line of sectionMatch[1].split(/\n/)) {
                const item = line.replace(/^[-*]\s+/, "").trim();
                if (item.length > 5)
...
export function extractCurrentErrors(messages) {
    const errors = [];
    for (const msg of messages) {
...
export function extractConstraints(messages) {
    const items = [];
    for (const msg of messages) {
        const text = extractTextContent(msg);
        const patterns = [
        ...
        const sectionMatch = text.match(/##\s*(?:constraints?|rules?)\s*\n((?:[-*]\s+.+\n?)+)/i);
        if (sectionMatch) {
            for (const line of sectionMatch[1].split(/\n/)) {
                const item = line.replace(/^[-*]\s+/, "").trim();
                if (item.length > 5)
...
export function extractFailedAttempts(messages) {
    const items = [];
    for (const msg of messages) {
        const text = extractTextContent(msg);
        const patterns = [
        ...
        const sectionMatch = text.match(/##\s*(?:failed attempts?|failed paths?|rejected)\s*\n((?:[-*]\s+.+\n?)+)/i);
        if (sectionMatch) {
            for (const line of sectionMatch[1].split(/\n/)) {
                const item = line.replace(/^[-*]\s+/, "").trim();
                if (item.length > 5)
