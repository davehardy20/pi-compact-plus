...
export function extractObjective(allMessages) {
    ...
    if (fullSubstantial)
        return fullSubstantial;
    return "Continue current task.";
}
export function findExplicitObjective(messages) {
...
export function findSubstantialObjective(messages) {
    for (let i = messages.length - 1; i >= 0; i--) {
        const msg = messages[i];
        if (msg.role === "user") {
            const text = extractTextContent(msg);
...
export function extractActiveFiles(messages) {
    const files = new Set();
...
export function extractBlockers(messages) {
    const blockers = [];
    for (const msg of messages) {
        const text = extractTextContent(msg);
        const lower = text.toLowerCase();
        const isError = msg.role === "toolResult"
            ? (msg.isError ?? false)
            : false;
        if (isError ||
...
export function extractNextStep(messages) {
    for (let i = messages.length - 1; i >= 0; i--) {
        const msg = messages[i];
        const text = extractTextContent(msg);
        const patterns = [
...
export function extractDependencyChain(messages, knownDecisions) {
    const chains = [];
    const patterns = [
    ...
    for (const decision of knownDecisions) {
        ...
        for (let i = 0; i < chains.length; i++) {
            ...
            if (chainNorm.includes(decisionNorm) ||
                decisionNorm.includes(chainNorm) ||
                chainNorm
                    .split(/\s+/)
                    .some((word) => decisionNorm.split(/\s+/).includes(word) && word.length > 4)) {
