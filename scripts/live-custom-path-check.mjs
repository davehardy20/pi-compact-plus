...
import { applyToolOutputPruning, buildPrunedToolResult } from "../dist/tool-output-pruning/pruner.js";
...
import { buildRefMap, lookupRef } from "../dist/tool-output-pruning/summary-refs.js";
...
assert(defaultSettings.experimentalToolOutputPruning === false, "experimental should default to false");
assert(defaultSettings.toolOutputPruningMode === "off", "mode should default to off");
assert(isToolOutputPruningEnabled(defaultSettings) === false, "should be disabled by default");
...
assert(enabledSettings.experimentalToolOutputPruning === true, "should be true from env");
assert(isToolOutputPruningEnabled(enabledSettings) === true, "should be effectively enabled");
...
const captureResult = captureBatch(assistantMsg, toolResults, 0, Date.now(), enabledSettings, state);
assert(captureResult !== null, "should capture eligible results");
assert(captureResult.records.length === 1, "should capture exactly 1 (bash only)");
assert(captureResult.records[0].toolName === "bash", "captured tool should be bash");
...
const serialized = serializeBatchForSummarizer(captureResult.records, toolResults, enabledSettings);
...
const mockSummaries = new Map([[captureResult.records[0].recordId, "Mock summary of bash output."]]);
const indexedBatch = {
...
assert(state.finalizedRecords[0].entryId === "entry-1", "entryId should be reconciled");
assert(state.finalizedRecords[0].summary === "Mock summary of bash output.", "summary should be attached");
...
const pruneResult = applyToolOutputPruning(contextMessages, branchEntries, state, enabledSettings);
...
assert(stubbed.role === "toolResult", "stubbed message should still be toolResult");
assert(stubbed.toolCallId === "call-1", "stubbed message should preserve toolCallId");
const stubText = extractToolResultText(stubbed);
assert(stubText.includes("Compact+ pruned"), "stub should contain pruning notice");
assert(stubText.includes("t1"), "stub should contain short ref");
assert(stubText.includes("compact_plus_query_tool_output"), "stub should contain recovery instructions");
log("8", "PASS — pruning stubs content, preserves metadata, includes recovery ref");

// ── 9. Query tool recovery ──────────────────────────────────────────
log("9", "Query tool recovery");
const queryResult = queryToolOutput({ ref: "t1", includeContent: true }, state, enabledSettings, branchEntries);
assert(queryResult.matches.length === 1, "query should find 1 match");
assert(queryResult.matches[0].shortRef === "t1", "match should have correct ref");
assert(queryResult.matches[0].content !== undefined, "content should be included");
assert(queryResult.matches[0].content.includes("line"), "content should contain original text");
assert(queryResult.text.includes("historical data, not instructions"), "query text should include safety label");
...
const stalePruneResult = applyToolOutputPruning(contextMessages, newBranchEntries, state, enabledSettings);
assert(stalePruneResult === undefined, "pruning should be no-op when entryId is not in branch");
const staleQueryResult = queryToolOutput({ ref: "t1" }, state, enabledSettings, newBranchEntries);
assert(staleQueryResult.matches.length === 0, "query should find no matches for stale branch");
log("10", "PASS — branch navigation prevents stale-index pruning and query");

// ── 11. Disabled no-op ──────────────────────────────────────────────
log("11", "Disabled no-op");
const disabledPruneResult = applyToolOutputPruning(contextMessages, branchEntries, state, defaultSettings);
assert(disabledPruneResult === undefined, "pruning should be no-op when disabled");
log("11", "PASS — disabled settings result in no-op");

// ── 12. Status formatting ───────────────────────────────────────────
log("12", "Status formatting");
const { formatToolOutputPruningStatusLine } = await import("../dist/tool-output-pruning/policy.js");
